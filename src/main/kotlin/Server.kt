...
//path params se id extract
    ...
    "/book" bind routes(
        ...
        "/id/{id}" bind GET to { req: Request ->
            ...
            val bookId: Int = getId(req)

            Response.invoke(OK).body(dao.getBookNameUsingBookId(bookId))
        },
        ...
        "/delete/{id}" bind DELETE to { req: Request ->
            Response(OK).body("${dao.delete(getId(req))}")
        }
    )
)

private fun getId(req: Request) = req.path("id")!!.toInt()
